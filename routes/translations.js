...
export default async function translationRoutes(fastify) {
  fastify.post('/translate/bulk', { preHandler: validateTranslationBody }, async (request, reply) => {
    ...
    try {
      ...
      if (!fs.existsSync(translationsPath)) {
        reply.code(400);
        return { success: false, error: `Translations for language "${targetLang}" are not available.` };
      }
      ...
      reply.code(500);
      return { success: false, error: 'Failed to translate texts.' };
    }
  });
  ...
  fastify.get('/', async (_request, reply) => {
    try {
      ...
      reply.code(500);
      return { success: false, error: 'Failed to fetch available translations.' };
    }
  });
}
