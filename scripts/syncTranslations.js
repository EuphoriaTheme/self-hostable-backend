...
const DEFAULT_SOURCE_BASE_URL = 'https://raw.githubusercontent.com/EuphoriaTheme/blueprint-translations/main';
...
function parseGithubRawUrl(rawBaseUrl) {
  ...
  const remainder = rawBaseUrl.slice(rawPrefix.length).split('/').filter(Boolean);
...
export async function syncBlueprintTranslations(options = {}) {
  ...
  const autoFetchEnabled = parseBoolean(process.env.TRANSLATIONS_AUTO_FETCH, true);

  if (!autoFetchEnabled) {
    logger.info('Translation auto-fetch is disabled (TRANSLATIONS_AUTO_FETCH=false).');
    return { skipped: true, reason: 'disabled' };
  }

  const sourceBaseUrl = (process.env.TRANSLATIONS_SOURCE_BASE_URL || DEFAULT_SOURCE_BASE_URL).replace(/\/$/, '');
  const remotePaths = parseCsv(process.env.TRANSLATIONS_REMOTE_PATHS);
  const sourcePaths = remotePaths.length > 0 ? remotePaths : DEFAULT_REMOTE_PATHS;
  const timeoutMs = Number(process.env.TRANSLATIONS_TIMEOUT_MS || DEFAULT_TIMEOUT_MS);
  const githubLookupEnabled = parseBoolean(process.env.TRANSLATIONS_GITHUB_TREE_LOOKUP, true);
  const translationsDir = path.join(__dirname, '../public/translations');

  await fs.mkdir(translationsDir, { recursive: true });

  const localFiles = (await fs.readdir(translationsDir)).filter(file => file.endsWith('.json'));
  ...
  for (const fileName of filesToSync) {
    ...
    for (const remotePath of sourcePaths) {
      ...
      try {
        ...
        if (!result.text) {
          logger.warn(`Could not fetch ${fileName} from ${sourceUrl} (HTTP ${result.status}).`);
          failed += 1;
          break;
        }
        ...
        logger.warn(`Failed fetching ${fileName} from ${sourceUrl}: ${error.message}`);
      }
    }

    if (!downloaded && githubFileMap && githubSource && githubFileMap.has(fileName)) {
      ...
      try {
        ...
        logger.warn(`GitHub fallback fetch failed for ${fileName}: ${error.message}`);
        failed += 1;
      }
    }
  ...
  logger.info(`Translation sync complete. Updated: ${updated}, Missing: ${missing}, Failed: ${failed}.`);
