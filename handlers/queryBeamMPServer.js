      ...
      if (responseData) {
        try {
          ...
          if (Array.isArray(parsedData.playerslist)) {
            players = parsedData.playerslist.filter(
          ...
          const maxplayersRaw =
            parsedData.maxplayers ??
            parsedData.maxPlayers ??
            parsedData.max_clients;
          const maxplayers = Number.isFinite(Number(maxplayersRaw))
            ? Number(maxplayersRaw)
            : 0;
          const numplayersRaw =
            parsedData.numplayers ??
            parsedData.players ??
            parsedData.playersCount;
          const numplayers = Number.isFinite(Number(numplayersRaw))
            ? Number(numplayersRaw)
            : players.length;
